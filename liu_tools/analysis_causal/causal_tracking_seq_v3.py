...
def load_tracking_signals(seq_dir):
    ...
    for file_path in file_list:
        with np.load(file_path) as data:
            ...
            H, W = heatmap.shape
            # 归一化到 [0, 1]，这里用 (index / (size-1))
            y_norm = y_idx / max(H - 1, 1)
            x_norm = x_idx / max(W - 1, 1)

            # Y = [conf, y_norm, x_norm]
            Y_seq.append([conf_val, y_norm, x_norm])
...
def build_windows(x, y, yr, n, dt):
    ...
    for t in range(n + dt, length):
        ...
        # 过去 Y：把 (n, 3) 展平为 (3n,) 特征
        data["Y_past"].append(y_past.reshape(-1))
        data["Yr_past"].append(yr_past.reshape(-1))
...
def analyze_tracking_causality(seq_dir, output_dir="results3"):
    ...
    window_N = 15  # 例如在最近 100 个样本上估一个局部 TE
