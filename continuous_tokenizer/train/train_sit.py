        ...
        for data_batch in train_dataloader:
            ...
            if args.dataset == 'imagenet_cached':
                ...
                with torch.no_grad() and accelerator.autocast():
                    ...
                    x, _, _ = vae.encode(raw_image)#batch_size,hidden_state,code_book_dim
                  
            y = y.to(device, non_blocking=True)
            z = None
            labels = y
            x = (x - latents_bias) / latents_scale# 对hidden进行归一化
            ...
            #当global_step为1或者sampling_steps的倍数时，进行验证，并使用可视化方式进行采样
            if (global_step == 1 or (global_step % args.sampling_steps == 0 and global_step > 0)):
                ...
                with torch.no_grad():
                    samples = euler_sampler(
                        ...
                        path_type=args.path_type,
                        heun=False,
                    ).to(torch.float32)#这里貌似再用扩散模型直接生成Encoder输出的latent state
                    samples = vae.decode(samples * latents_scale + latents_bias)                    
                    samples = torch.clamp(samples, -1.0, 1.0)
                    samples = (samples + 1) / 2
                    
                out_samples = accelerator.gather(samples.to(torch.float32))#使用gather将所有的数据收集到主进程
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--batch-size", type=int, default=256)#ori 256
