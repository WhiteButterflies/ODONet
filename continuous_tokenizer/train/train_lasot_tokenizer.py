...
class CustomImageDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        self.images = []
        for root, dirs, files in os.walk(os.path.join(data_dir,"val_input")):
            for file in files:
    ...
    def __getitem__(self, index):
        image_x_path = self.images[index]
        image_y_path = self.images[index].replace('val_input', 'val_output')
        image_x = Image.open(image_x_path).convert('RGB')
        image_y = Image.open(image_y_path).convert('RGB')
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            imgs_y = y.to(device, non_blocking=True)

            # generator training
            optimizer.zero_grad()
            with torch.cuda.amp.autocast(dtype=ptdtype):  
                recons_imgs, codebook_loss, info = vq_model(imgs)
                loss_gen = vq_loss(codebook_loss, imgs_y, recons_imgs, optimizer_idx=0, global_step=train_steps+1,
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                #imgs
                loss_disc = vq_loss(codebook_loss, imgs_y, recons_imgs, optimizer_idx=1, global_step=train_steps+1,
                                    logger=logger, log_every=args.log_every)
