...
from .quantizers.softvq import SoftVectorQuantizer,SoftVectorQuantizerOffset
...
class VQModel(nn.Module, PyTorchModelHubMixin):
    ...
    def encode(self, x):
        h = self.encoder(x)#BS,hidden_nums,embed_dim
        h = self.quant_conv(h)
        quant, emb_loss, info = self.quantize(h)#使用codebook
    ...
    def forward_dec_quant(self, input):
        b, _, h, w = input.size()
        quant, diff, info = self.encode(input)
        self.quant = quant
        dec_quant = self.post_quant_conv(quant)#for upped channel，现在放弃使用
        return dec_quant
...
def VectorQuantizer_Head(config):
    return SoftVectorQuantizer(config['codebook_size'], config['codebook_embed_dim'],
                                            config['entropy_loss_ratio'],
                                            config['tau'],
                                            config['num_codebooks'],
                                            config['codebook_l2_norm'], config['codebook_show_usage'])
def SoftVectorQuantizerOffset_head(config):
    return SoftVectorQuantizerOffset(
...
VQ_models = {
    ...
    'SoftVQ': SoftVQ,
    # 'VQHead': VectorQuantizer_Head
    }


if __name__ == '__main__':
    AEModel.from_pretrained("MAETok/maetok-b-128")
