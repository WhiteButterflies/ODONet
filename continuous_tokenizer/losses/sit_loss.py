...
class SILoss:
    ...
    def __call__(self, model, images, model_kwargs=None, zs=None):
        ...
        model_input = alpha_t * images + sigma_t * noises#是加噪的隐藏层model_input的获取，即latent+高斯噪声
        if self.prediction == 'v':
            model_target = d_alpha_t * images + d_sigma_t * noises
        else:
            raise NotImplementedError() # TODO: add x or eps prediction
        #这个是DiT的forward方法，输入的是加噪的隐藏层model_input，输出的是预测的输出model_output
        model_output, zs_tilde  = model(model_input, time_input.flatten(), **model_kwargs)
        denoising_loss = mean_flat((model_output - model_target) ** 2)#实际上是一种有环图，保证任意两个时刻都可以正向（正向过程）和反向（模型参数）互相转化
