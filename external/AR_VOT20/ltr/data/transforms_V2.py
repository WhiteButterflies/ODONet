...
class Transform_V2:
    ...
    Args of constructor:
        transforms: An arbitrary number of transformations, derived from the TransformBase_V2 class.
    ...
    The following are outputs:
        ...
        bbox  -  Transformed bounding box
        mask  -  Transformed segmentation mask
        roll  -  Random roll used for the transformation
    ...
    def merge_dicts(self,dict_list):
        if not dict_list:  # 如果列表为空，返回空字典
            return {}

        result = {}
        keys = dict_list[0].keys()  # 以第一个字典的键为准

        for key in keys:
            result[key] = [d[key] for d in dict_list if key in d]  # 确保键存在

        return result

    def __call__(self, **inputs):
        var_names = [k for k in inputs.keys() if k in self._valid_inputs]
        # make sure add new roll added by liu
        var_names.append('roll')
        ...
        if not joint_mode:
            ...
            out = []
            for inp in self._split_inputs(inputs):
                for t in self.transforms:
                    inp = {k: v for k, v in inp.items() if k in self._valid_inputs}
                    inp = t(**inp, joint=False, new_roll=new_roll)
                out.append(inp)

            out = self.merge_dicts(out)
...
class TransformBase_V2:
    ...
    def __call__(self, **inputs):
        ...
        #added by liu for exoort roll
        if input_args.get('joint',False):
            outputs['roll'] = list(self._rand_params)*len(input_vars['image']) if 'image' in input_vars else self._rand_params
        else:
            outputs['roll'] = list(self._rand_params)
        return outputs
    ...
    def transform_bbox(self, bbox, image_shape, *rand_params):
        ...
        if self.transform_coords.__code__ == TransformBase_V2.transform_coords.__code__:
            return bbox
...
class ToTensor(TransformBase_V2):
    """Convert to a Tensor"""
...
class ToTensorAndJitter(TransformBase_V2):
...
class Normalize(TransformBase_V2):
    """Normalize image"""
    ...
    def transform_image(self, image,do=None):
...
class ToGrayscale(TransformBase_V2):
...
class RandomHorizontalFlip(TransformBase_V2):
