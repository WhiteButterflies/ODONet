...
import traceback
from liu_tools import FileListMemoryMapper
...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k', 'lasot', 'lasot_extension_subset', 'otb', 'uav', 'nfs', 'tnl2k','vasttrack']:
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8,mapper=None,mode_dict = {}):
    ...
    if debug:
        ...
        try:
            output = tracker.run_sequence(seq, debug=debug,mapper=mapper,mode_dict = mode_dict)
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        ...
        #FileListMemoryMapper([seq])
        param_list = [(seq, tracker_info, debug, num_gpus,None) for seq, tracker_info in product(dataset, trackers)]
        with multiprocessing.Pool(processes=threads) as pool:
            pool.starmap(run_sequence, param_list)
    elif mode =='liu':
        ctx = multiprocessing.get_context('spawn')
        param_list = [(seq, tracker_info, debug, num_gpus,None) for seq, tracker_info in product(dataset, trackers)]
        with ctx.Pool(processes=min(threads, num_gpus)) as pool:
            pool.starmap(run_sequence, param_list)
    print('Done')
