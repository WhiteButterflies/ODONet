...
from lib.models.odonet_v1_odop import build_odonet
...
class ODONET(BaseTracker):
    ...
    def track(self, image, info: dict = None):
        ...
        if self.debug == 2:
            ...
            # # cv2.rectangle(image_t_1, (int(x1*(self.fx_sz**2)), int(y1*(self.fx_sz**2)), int((x1+w)*(self.fx_sz**2)), int((y1+h)*(self.fx_sz**2))), color=(0, 0, 255), thickness=2)
            #
            # # x1, y1, w, h=self.now_box_crop.cpu().numpy() #差一个resize_factor
            # image_t = cv2.cvtColor(self.search_img_list[-1], cv2.COLOR_RGB2BGR)
            # # cv2.rectangle(image_t, (int(x1*(self.fx_sz**2)), int(y1*(self.fx_sz**2)), int((x1+w)*(self.fx_sz**2)), int((y1+h)*(self.fx_sz**2))), color=(0, 0, 255), thickness=2)
