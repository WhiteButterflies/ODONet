...
class Preprocessor_wo_mask(object):
    ...
    def process(self, img_arr: np.ndarray):#将numpy图片转化成tensor，并作归一化。
        # Deal with the image patch
        img_tensor = torch.tensor(img_arr).cuda().float().permute((2,0,1)).unsqueeze(dim=0)#调整维度顺序并增加维度
        img_tensor_norm = ((img_tensor / 255.0) - self.mean) / self.std  # (1,3,H,W)
...
def vis_attn_maps(attn_weights, q_w, k_w, skip_len, x1, x2, x1_title, x2_title, frame_id,save_path='.', idxs=None):
    ...
    for attn in attn_weights:
        #attn_weights_mean.append(attn[..., :q_w**2,q_w**2:].mean(dim=1).squeeze().reshape(shape2).cpu())
        # attn_weights_mean.append(attn[..., :q_w ** 2, q_w ** 2:].mean(dim=1).squeeze().reshape(shape1+shape2).cpu())
        attn_weights_mean.append(attn[..., k_w ** 2:q_w ** 2+k_w ** 2, :k_w ** 2].mean(dim=1).squeeze().reshape(shape1+shape2).cpu())
        # attn_weights_mean.append(attn[..., :k_w ** 2,k_w ** 2:q_w ** 2+k_w ** 2 ].mean(dim=1).squeeze().reshape(shape1+shape2).cpu())
    ...
    for attn_weight in attn_weights_mean:
        ...
        plt.savefig(save_path + '/{}-Block{}_attn_weight.png'.format(frame_id,block_num))
        plt.close()
        block_num += 1
