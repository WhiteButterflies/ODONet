...
class ODONET(BaseTracker):
    ...
    def initialize(self, image, info: dict):
        ...
        '''ended'''
        '''added by liu for Kalman Filter'''
        # [添加] 初始化卡尔曼滤波器
        self.kf = SimpleKF()
        init_x, init_y, init_w, init_h = info['init_bbox']
        init_cx = init_x + init_w / 2
        init_cy = init_y + init_h / 2
        self.kf.x[:4, 0] = [init_cx, init_cy, init_w, init_h]
        '''ended by liu'''
    ...
    def track(self, image, info: dict = None):
        ...
        '''use kalman Filter replace MLP'''
        # [修改] 使用 Kalman Filter 预测偏移覆盖网络输出
        self.kf.predict()
        kf_pred = self.kf.x[:4, 0]  # KF 预测的全局坐标 [cx, cy, w, h]
        # 将 cx, cy, w, h 转换为 x, y, w, h
        kf_pred_state = [kf_pred[0] - kf_pred[2] / 2, kf_pred[1] - kf_pred[3] / 2, kf_pred[2], kf_pred[3]]
        # 把全局坐标的预测框，映射回当前 search crop 坐标系下，保持跟原 MLP 逻辑一致
        kf_pred_box_crop = transform_image_to_crop(
            torch.tensor(kf_pred_state),
            torch.tensor(self.prev_state),
            resize_factor,
            torch.Tensor([self.params.search_size, self.params.search_size]),
            normalize=True
        )
        # 转换为 cx, cy, w, h 格式计算归一化的 offset
        kf_pred_cxcywh = box_xyxy_to_cxcywh(box_xywh_to_xyxy(kf_pred_box_crop))
        prev_cxcywh = box_xyxy_to_cxcywh(box_xywh_to_xyxy(self.prev_box_crop.cpu()))
        # 得到新偏移并覆盖给 delta_xywh
        self.delta_xywh = (kf_pred_cxcywh - prev_cxcywh).unsqueeze(0).unsqueeze(0).cuda().to(torch.float32)
        '''ended by liu'''
        ...
        '''added by liu for KalFilter'''
        # [添加] 更新 Kalman Filter
        res_x, res_y, res_w, res_h = self.state
        res_cx = res_x + res_w / 2
        res_cy = res_y + res_h / 2
        z = np.array([[res_cx, res_cy, res_w, res_h]]).T
        self.kf.update(z)
        '''ended by liu'''
