...
from lib.train.dataset import Otb99_lang, Tnl2k, RefCOCOSeq,Otb
from lib.train.data import sampler, opencv_loader, processing, LTRLoader,TestLoader
...
def update_settings(settings, cfg):
    ...
    settings.shared_search_jitter = getattr(cfg.DATA, "SHARED_SEARCH_JITTER", False)
...
def build_dataloaders(cfg, settings):
    ...
    dataset_train.search_near_gap = getattr(cfg.DATA, "SEARCH_NEAR_GAP", None)
...
def build_train_prev_dataloaders(cfg, settings):
    settings.num_template = getattr(cfg.DATA.TEMPLATE, "NUMBER", 1)
    settings.num_search = getattr(cfg.DATA.SEARCH, "NUMBER", 1)
    ovtrack_cfg = getattr(cfg.MODEL, "OVTRACK", None)
    num_partial_search = 0
    if ovtrack_cfg is not None and bool(getattr(ovtrack_cfg, "USE_STREAM_INPUT", False)):
        num_partial_search = int(getattr(ovtrack_cfg, "NUM_PARTIAL_SEARCH", 0))
    num_sampler_search = settings.num_search + num_partial_search
    # Data transform
    transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.05),
                                    tfm.RandomHorizontalFlip(probability=0.5))

    transform_train = tfm.Transform(tfm.ToTensorAndJitter(0.2),
                                    tfm.RandomHorizontalFlip_Norm(probability=0.5),#默认0.5 但是需要改成0
                                    tfm.Normalize(mean=cfg.DATA.MEAN, std=cfg.DATA.STD))
    ...
    data_processing_train = processing.Train_Prev_Processing(search_area_factor=search_area_factor,
                                                          output_sz=output_sz,
                                                          center_jitter_factor=settings.center_jitter_factor,
                                                          scale_jitter_factor=settings.scale_jitter_factor,
                                                          mode='sequence',
    ...
    data_processing_val = processing.Train_Prev_Processing(search_area_factor=search_area_factor,
                                                          output_sz=output_sz,
                                                          center_jitter_factor=settings.center_jitter_factor,
                                                          scale_jitter_factor=settings.scale_jitter_factor,
                                                          mode='sequence',
    ...
    sampler_mode = getattr(cfg.DATA, "SAMPLER_MODE", "causal")
    # print("sampler_mode", sampler_mode)
    dataset_train = sampler.Tracking_train_Sampler(datasets=names2datasets(cfg.DATA.TRAIN.DATASETS_NAME, settings, opencv_loader),
                                            p_datasets=cfg.DATA.TRAIN.DATASETS_RATIO,
                                            samples_per_epoch=cfg.DATA.TRAIN.SAMPLE_PER_EPOCH,
                                            max_gap=cfg.DATA.MAX_SAMPLE_INTERVAL, num_search_frames=num_sampler_search,
                                            num_template_frames=settings.num_template, processing=data_processing_train,
                                            frame_sample_mode=sampler_mode, max_query_len=settings.max_query_len,
                                            bert_model=settings.bert_model, bert_path=settings.bert_path,
                                            multi_modal_language=settings.multi_modal_language
    ...
    loader_train = LTRLoader('train', dataset_train, training=True, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=shuffle,
                             num_workers=cfg.TRAIN.NUM_WORKER, drop_last=True, stack_dim=1, sampler=train_sampler)
    dataset_val = sampler.Tracking_train_Sampler(datasets=names2datasets(cfg.DATA.VAL.DATASETS_NAME, settings, opencv_loader),
    ...
    dataset_val.search_near_gap = getattr(cfg.DATA, "SEARCH_NEAR_GAP", None)
...
def build_train_prev_dataloaders_v3(cfg, settings):
    settings.num_template = getattr(cfg.DATA.TEMPLATE, "NUMBER", 1)
    settings.num_search = getattr(cfg.DATA.SEARCH, "NUMBER", 1)
    # Data transform
    transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.05),
    ...
    data_processing_train = processing.Train_Prev_Processing_V3(search_area_factor=search_area_factor,
                                                          output_sz=output_sz,
                                                          center_jitter_factor=settings.center_jitter_factor,
                                                          scale_jitter_factor=settings.scale_jitter_factor,
                                                          mode='sequence',
    ...
    data_processing_val = processing.Train_Prev_Processing_V3(search_area_factor=search_area_factor,
                                                          output_sz=output_sz,
                                                          center_jitter_factor=settings.center_jitter_factor,
                                                          scale_jitter_factor=settings.scale_jitter_factor,
                                                          mode='sequence',
...
def build_test_dataloaders(cfg, settings):
    settings.num_template = getattr(cfg.DATA.TEMPLATE, "NUMBER", 1)
    settings.num_search = getattr(cfg.DATA.SEARCH, "NUMBER", 1)
    # Data transform
    transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.05),
    ...
    data_processing_train = processing.TestProcessing(search_area_factor=search_area_factor,
                                                          output_sz=output_sz,
                                                          center_jitter_factor=settings.center_jitter_factor,
                                                          scale_jitter_factor=settings.scale_jitter_factor,
                                                          mode='sequence',
    ...
    sampler_mode = getattr(cfg.DATA, "SAMPLER_MODE", "causal")
    # print("sampler_mode", sampler_mode)
    dataset_train = sampler.Tracking_test_Sampler(
    ...
    loader_train = TestLoader('train', dataset_train, training=True, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=shuffle,
                             num_workers=cfg.TRAIN.NUM_WORKER, drop_last=False, stack_dim=1, sampler=train_sampler)
