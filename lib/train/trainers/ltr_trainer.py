...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        '''added by liu for regist a hook'''
        #
        # def grad_hook(grad, name):
        #     print(f"Gradient norm for {name}: {grad.norm(2).item()}")
        #
        # # 为 deform_attention 模块中的每个参数注册 hook
        # for name, param in self.actor.net.module.neck.interactions[0].deform_attention.named_parameters():
        ...
        for i, data in enumerate(tqdm(loader), 1):
            ...
            if loader.training:
                ...
                if not self.use_amp:
                    ...
                    # 2. 如果开启了梯度裁剪
                    if self.settings.grad_clip_norm > 0:
                        # 必须先将优化器中参数的梯度反缩放（unscale）
                        self.scaler.unscale_(self.optimizer)
                        # 然后再像往常一样进行梯度裁剪
                        torch.nn.utils.clip_grad_norm_(self.actor.net.parameters(), self.settings.grad_clip_norm)
                    # 3. 步进优化器（内部会自动检查是否已经 unscale 过，不会重复操作）
