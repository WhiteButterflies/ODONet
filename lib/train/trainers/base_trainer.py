...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    save_every_epoch = getattr(self.settings, "save_every_epoch", False)#modifed by liu ,ori is false
                    # save every 10 epochs
                    # save_every_epoch = True
                    if epoch > (max_epochs - 5) or save_every_epoch or epoch % 20 == 0 or epoch==1 or epoch ==5:
    ...
    def save_checkpoint(self):
        ...
        net_type = type(net).__name__.upper() #修改其以保证训练和测试均接受相同名称
        state = {
            'epoch': self.epoch,
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        actor_type = type(self.actor).__name__
        net_type = type(net).__name__.upper() #与保存时的名称一致，对应的是save_checkpoint的net_type
        ...
        # Load network
        checkpoint_dict = torch.load(checkpoint_path, map_location='cpu',weights_only=False)# pytorch 2.6,weights_only=False
        ...
        for key in fields:
            if key in ignore_fields:
                continue
            if key == 'net':
                net.load_state_dict(checkpoint_dict[key],strict=False) #default strict=True
            elif key == 'optimizer':
