...
class Otb(BaseVideoDataset):
    ...
    def _build_sequence_list(self, vid_ids=None, split=None):
        if split is not None:
            ...
            if split == 'train':
                ...
                file_path = os.path.join(ltr_path, 'data_specs', 'otb_test_split.txt')
            else:
    ...
    def _get_frame_path(self, seq_path, frame_id):
        if seq_path.split('/')[-1] != 'Board':
            return os.path.join(seq_path, 'img', '{:04}.jpg'.format(frame_id + 1))
        else:
            return os.path.join(seq_path, 'img', '{:05}.jpg'.format(frame_id + 1))
    ...
    def _get_class(self, seq_path):
        ...
        return raw_class
    def _get_seq_name(self, seq_path):
        raw_seq_name = seq_path.split('/')[-1]
        return raw_seq_name
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        obj_class = self._get_class(seq_path)
        obj_seq_name = self._get_seq_name(seq_path)
