...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                elif self.frame_sample_mode =='train_prev_order':
                    template_frame_ids,search_frame_ids,prev_search_ids = self.get_frame_ids_testing_order(visible)
    ...
    def get_frame_ids_order(self, visible):
        ...
        while (None in frame_ids) or (len(frame_ids) == 0):
            base_frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=0,  # used for base
                                                     max_id=len(visible))
            frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames + self.num_search_frames,
                                                 # 从base_frame_id开始往前后找
                                                 min_id=base_frame_id[0] - self.max_gap - gap_increase,
            ...
            if (frame_ids is None) or (None in frame_ids):
                ...
                if gap_increase > 1000:
                    print("too large image gap, check the sampler, current gap: " + str(gap_increase))
                continue
            if torch.rand(1) < 0.5:  # randomly choose the order of the frames，50%正向的模板和搜索区域，50%反向的模板和搜索区域
    ...
    def random_frames_with_random_gap(self,id_list, gap, order="asc"):
        """
        id_list: 原始 ID 列表，例如 [32, 18, 68]
        gap: 输出列表中 max(frame) - min(frame) 的最大允许值
        ...
        max_possible_span = min(gap, max_id - min_id)
        # 情况 1：可以满足 GAP 限制
        if max_possible_span >= n - 1:
            # 随机决定本次输出列表的总跨度
            span = random.randint(n - 1, max_possible_span)
            # 随机决定起始帧
            start = random.randint(min_id, max_id - span)
            end = start + span
            # 在 start 和 end 中间随机选 n-2 个点
            middle = random.sample(range(start + 1, end), n - 2)
            frames = [start] + sorted(middle) + [end]
        # 情况 2：无法满足 GAP，则直接随机生成 n 个有序帧号
        else:
            # print(id_list)
            if total_range >= n:
                # 不重复随机生成
                frames = random.sample(range(min_id, max_id + 1), n)
                frames.sort()
            else:
                # 如果范围连 n 个不重复值都不够，则允许重复
                frames = [random.randint(min_id, max_id) for _ in range(n)]
                frames.sort()
        if order == "desc":
            frames.reverse()
        return frames

    def get_frame_ids_testing_order(self, visible):
        # get template and search ids in an 'order' manner, the template and search regions are arranged in chronological order
        frame_ids = []
        gap_increase = 0
        while (None in frame_ids) or (len(frame_ids)==0):
            base_frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=0,  #used for base
                                                     max_id=len(visible))
            frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames+self.num_search_frames,# 从base_frame_id开始往前后找
                                                      min_id=base_frame_id[0] - self.max_gap - gap_increase,
                                                      max_id=base_frame_id[0] + self.max_gap + gap_increase)
            if (frame_ids is None) or (None in frame_ids):
                gap_increase += 5
            ...
            if torch.rand(1) < 0.5: # randomly choose the order of the frames，50%正向的模板和搜索区域，50%反向的模板和搜索区域
                frame_ids.sort(reverse=True)
                template_frame_ids = frame_ids[0:self.num_template_frames]
                prev_search_frame_ids = frame_ids[self.num_template_frames:]
                search_frame_ids = self.random_frames_with_random_gap(prev_search_frame_ids,4,'desc')

            else:
                frame_ids.sort(reverse=False)
                template_frame_ids = frame_ids[0:self.num_template_frames]
                prev_search_frame_ids = frame_ids[self.num_template_frames:]
                search_frame_ids = self.random_frames_with_random_gap(prev_search_frame_ids, 4, 'asc')

            # Increase gap until a frame is found
            gap_increase += 5
            if gap_increase > 1000:
                print("too large image gap, check the sampler, current gap: " + str(gap_increase))
        return template_frame_ids, search_frame_ids,prev_search_frame_ids
...
class Tracking_train_Sampler(torch.utils.data.Dataset):
    """ Class responsible for sampling frames from training sequences to form batches.
    专门为了从上一帧采样偏移量，避免分布差异过大
    采样mode为 train_prev_order
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    template_frame_ids,search_frame_ids,prev_search_frame_ids = self.get_frame_ids_testing_order(visible)
                elif self.frame_sample_mode == "trident" or self.frame_sample_mode == "trident_pro":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_trident(visible)
                elif self.frame_sample_mode == "stark":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_stark(visible, seq_info_dict["valid"])
            ...
            try:
                ...
                if self.frame_sample_mode =='train_prev_order':
                    prev_search_frames, prev_search_anno, prev_meta_obj_test = dataset.get_frames(seq_id, prev_search_frame_ids,
                                                                                   seq_info_dict)
                    # prev_search_masks = prev_search_anno['mask'] if 'mask' in prev_search_anno else [torch.zeros(
                    #     (H, W))] * self.num_search_frames

                    data['prev_search_anno'] = prev_search_anno['bbox']
                    # data['prev_search_masks'] = prev_search_masks
                    # data['prev_search_images'] = prev_search_frames
        ...
        while (None in frame_ids) or (len(frame_ids)==0):
            ...
            if (frame_ids is None) or (None in frame_ids):
                ...
                if gap_increase > 1000:
                    print("too large image gap, check the sampler, current gap: " + str(gap_increase))
                continue
            desc = bool(torch.rand(1) < 0.5)   # 50% 正向 / 50% 反向 的模板-搜索时序
            frame_ids.sort(reverse=desc)
            template_frame_ids = frame_ids[0:self.num_template_frames]
            search_frame_ids = frame_ids[self.num_template_frames:]
            # 可选:把"搜索帧"夹到一个近邻簇(跨度 <= search_near_gap),而模板仍可落在 [base±max_gap] 任意处。
            # 临近帧 => 帧间位移小 => OT 关联/offset 的小运动近似成立,亦可支持 row-reuse 省一次前向。
            near_gap = getattr(self, 'search_near_gap', None)
            if near_gap is not None and near_gap > 0 and len(search_frame_ids) > 0:
                s0 = search_frame_ids[0]
                near = self._sample_visible_ids(visible, num_ids=self.num_search_frames,
                                                min_id=s0, max_id=s0 + near_gap + gap_increase)
                if near is not None and None not in near:
                    near.sort(reverse=desc)
                    search_frame_ids = near
            prev_search_frame_ids = [
                random.randint(
                    min(template_frame_ids[-1], search_frame_ids[i]),
                    max(template_frame_ids[-1], search_frame_ids[i])
                )
...
class Tracking_test_Sampler(torch.utils.data.Dataset):
    """ Class responsible for sampling frames from training sequences to form batches.
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        '''added by liu for seq id 用来保证独一无二的采样'''
        self.seq_id = int(os.environ['LOCAL_RANK'])
        ...
        '''added bu liu for memory dataset sequence if not used'''
        self.dataset_seq_dict = {}
        for idx,dataset in enumerate(self.datasets):
            self.dataset_seq_dict[dataset.get_name()] = []
            for seq_id in range(dataset.get_num_sequences()):
                self.dataset_seq_dict[dataset.get_name()].append({str(seq_id):0})
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            # sample a sequence from the given dataset
            seq_id, visible, seq_info_dict = self.sample_seq_from_dataset(dataset, is_video_dataset)
            self.seq_id = seq_id + int(torch.distributed.get_world_size())
            #保证可以循环采样
            if self.seq_id >= dataset.get_num_sequences():
                self.seq_id = int(os.environ['LOCAL_RANK'])
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    template_frame_ids, search_frame_ids = self.get_frame_ids_order(visible)
                elif self.frame_sample_mode == "all_seq":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_seq_order(visible)
                elif self.frame_sample_mode == "trident" or self.frame_sample_mode == "trident_pro":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_trident(visible)
                elif self.frame_sample_mode == "stark":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_stark(visible, seq_info_dict["valid"])
            ...
            try:
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'dataset': dataset.get_name(),
                                   'test_class': meta_obj_test.get('object_class_name'),
                                   'seq_name':meta_obj_test.get('object_seq_name'),
                                   'valid':True
    ...
    def get_frame_ids_order(self, visible):
        ...
        while (None in frame_ids) or (len(frame_ids)==0):
            ...
            if gap_increase > 1000:
                print("too large image gap, check the sampler, current gap: " + str(gap_increase))
        return template_frame_ids, search_frame_ids
    def get_frame_ids_seq_order(self, visible):
        # get template and search ids in an 'order' manner, the template and search regions are arranged in chronological order
        template_frame_ids = [0]
        search_frame_ids =list(range(len(visible)))
