...
class DAttentionBaseline(nn.Module):
    ...
    def __init__(
        ...
        no_off, fixed_pe, ksize, log_cpb,tracking
    ):
    ...
    #added by liu for 根据关键点提取特征
    # @torch.no_grad()
    def extract_features(self,feature_map: torch.Tensor, coords: torch.Tensor) -> torch.Tensor:
        """
        从特征图中提取多组归一化坐标对应的特征。

        参数:
            feature_map: 形状为 [B, C, H, W] 的特征图
            coords: 形状为 [B, N, 2] 的归一化坐标 (x, y)，范围 [0, 1]
        ...
        # 将归一化坐标 [0,1] 转换为 grid_sample 的输入范围 [-1,1]
        grid = coords * 2 - 1  # 形状 [B, N, 2]

        # 调整 grid 的形状为 [B, N, 1, 2]（F.grid_sample 要求最后一维是2）
        grid = grid.view(B, N, 1, 2).to(feature_map.device)

        # 用双线性插值提取特征
        # 输入 feature_map: [B, C, H, W], grid: [B, N, 1, 2]
        # 输出: [B, C, N, 1]
        ...
        # 调整形状为 [B, N, C]
        features = features.permute(0, 2, 3, 1).squeeze(2)  # [B, N, 1, C] -> [B, N, C]
        # 添加特征归一化
    ...
    def forward(self, x,points=None):
        ...
        offset = self.conv_offset(q_off).contiguous()  # B * g 2 Hg Wg   #整体进行卷积，对应Fig2的Offsets的生成
        ...
        if self.tracking:
            offset = 2*offset
            offline_offset =offset



        if self.training==False:
            offset = 0.5*offset+0.5*self.offset_online
            # offset = 0*offset+-100*self.offset_online
            # offset = 1*offset
            # offset = -2*offset
        ...
        if self.offset_range_factor >= 0:
            ...
            pos = (offset + reference).clamp(-1., +1.)# Fig2 的网格坐标（上面）和下面的offset进行加和

        if self.no_off:
            x_sampled = F.avg_pool2d(x, kernel_size=self.stride, stride=self.stride)
            assert x_sampled.size(2) == Hk and x_sampled.size(3) == Wk, f"Size is {x_sampled.size()}"
        else:
            x_sampled = F.grid_sample(#根据pos重新调整x的元素的组成
        ...
        if points is None:
            sampled_x_point = None
        else:
            sampled_x_point = self.extract_features(out,points)

        y = self.proj_drop(self.proj_out(out))

        # return y, pos.reshape(B, self.n_groups, Hk, Wk, 2), reference.reshape(B, self.n_groups, Hk, Wk, 2)
        return y, [offset,sampled_x_point,offline_offset]
        # return y
