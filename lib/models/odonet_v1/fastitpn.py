    ...
    def forward(self, x):
        if self.gamma_2 is None:
            if self.postnorm:
                ...
                x = residual * self.alpha + x
                x = self.norm2(x)
            else:# 注 除第一层的drop_path为恒等变换外，其余的drop_path均为随机失活，结果可能随着不同的推理次数而有所不同
...
class Fast_iTPN(nn.Module):
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list):
        ...
        z = self.patch_embed(z)#bn,c,h,w-->bn,c2,h2,w2
        x = self.patch_embed(x)#patch_embed 缩减分辨率到1/4
        # forward stage1&2
        if not self.convmlp and self.stop_grad_conv1:
            x = x.detach() * 0.9 + x * 0.1

        for blk in self.blocks[:-self.num_main_blocks]:#经过这里再一次缩减为1/4，并在未来均使用这个个数作为基准
        ...
        z = z.flatten(2).transpose(1, 2)  #注意 token本来应该是HXW ，但是Flatten了每个模板token个数只有H or W 个
