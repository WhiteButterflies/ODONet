...
class InteractionBlock(nn.Module):
    def __init__(self, d_model, extra_extractor, extra_deform_attention,grad_ckpt):
        super().__init__()
    ...
    def forward(self,x,xs,blocks):
        ...
        if self.deform_attention is not None:
            ...
            xs ,offset = self.deform_attention(xs.permute(0, 2, 1).contiguous().view(B, C, H, W))
...
class Mamba_Neck(nn.Module):
    def __init__(self, in_channel=512,d_model=512,d_inner=1024,bias=False,n_layers=4,dt_rank=32,d_state=16,d_conv=3,dt_min=0.001,
        ...
        self.interactions = nn.ModuleList([  #pay attention added extra by manual
            InteractionBlock(d_model=d_model,extra_extractor=(True if i == n_layers - 1 else False),extra_deform_attention=(True if i in [2] else False),grad_ckpt=grad_ckpt)
    ...
    def forward(self, x,xs,h,blocks,interaction_indexes):
        ...
        for i,index in enumerate(interaction_indexes):
            ...
            x,xs,h[i] = self.interactions[i](x,xs,blocks[index[0]:index[1]])#Figure 2 b other part ,h[i] means offset
